...
class AddDataConfigView(QWidget, Ui_data_config_widget):
    ...
    def __init__(self, parent=None):
        ...
        self.buttonGroup.buttonClicked.connect(self.radio_toggled)
    ...
    def add_data_config(self):
        """ 添加数据表配置 """
        # 常规配置缺失时, 提示错误
        if not self.line_data_config_name.text() \
                or not self.line_channel_id.text() \
                or not self.line_type_id.text() \
                or not self.line_sheet.text() \
                or not self.line_customer_col.text()\
                or (not self.radio_special_none.isChecked() and not self.line_special.text()):
            InfoBar.error(
                title='配置错误',
                content="配置项不能为空!",
        ...
        elif self.line_data_config_name.text().__contains__(' ') \
                or self.line_channel_id.text().__contains__(' ') \
                or self.line_type_id.text().__contains__(' ') \
                or self.line_sheet.text().__contains__(' ') \
                or self.line_customer_col.text().__contains__(' ') \
                or self.line_special.text().__contains__(' '):
            InfoBar.error(
                title='配置错误',
                content="配置项不能含有空白字符!",
                orient=Qt.Orientation.Horizontal,
        ...
        # 配置名重复时, 提示错误
        ...
        elif not self.radio_special_none.isChecked() and self.line_special.text().replace("，", ",").split(",").__contains__(''):
            InfoBar.error(
                title='配置错误',
                content="指定列/排除列配置错误, 请检查多余的逗号!",
                orient=Qt.Orientation.Horizontal,
                isClosable=False,  # disable close button
                position=InfoBarPosition.TOP_RIGHT,
                duration=2000,
        ...
        elif not self.radio_special_none.isChecked() and len(self.line_special.text().replace("，", ",").split(",")) != len(set(self.line_special.text().replace("，", ",").split(","))):
            InfoBar.error(
                title='配置错误',
                content="不允许 指定列/排除列 列名重复!",
                orient=Qt.Orientation.Horizontal,
                isClosable=False,  # disable close button
                position=InfoBarPosition.TOP_RIGHT,
                duration=2000,
            ...
            self.data_config[f'{self.line_data_config_name.text()}'] = {
                ...
                "special_type": self.buttonGroup.checkedButton().text(),
                "special_col": self.line_special.text().replace("，", ",").upper().split(",")
            }
    ...
    def radio_toggled(self):
        if self.radio_special_none.isChecked():
            self.label_special.setText('无特殊需求:')
            self.line_special.setText('')
            self.line_special.setPlaceholderText('')
            self.line_special.setEnabled(False)
        elif self.radio_special_contain.isChecked():
            self.label_special.setText('指定列(导出):')
            self.line_special.setPlaceholderText('填写指定导出的列号,逗号隔开。例如: C,D,E,F')
            self.line_special.setEnabled(True)
        else:
            self.label_special.setText('排除列(不导出):')
            self.line_special.setPlaceholderText('填写不要导出的列号,逗号隔开。例如: C,D,E,F')
            self.line_special.setEnabled(True)
